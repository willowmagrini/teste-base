...
export function QueryPanel ({ attributes, setAttributes, title = __('Query', 'teste-base') }) {
    ...
    const { data: postTypes } = useRestApi('teste-base/v2/post_types');
    ...
    return (
        <PanelBody className="teste-base-gutenberg-panel__panel-body" title={title}>
            <PanelRow>
                <SelectPostType
                    required={true}
                    value={postType}
            ...
            {(postTypes?.[postType]?.hierarchical) ? (
                <PanelRow>
                    <ToggleControl
                        label={__('Show children posts?', 'teste-base')}
            ...
            <PanelRow>
                <h2>{__('Filter posts to not display', 'teste-base')}</h2>
            </PanelRow>
