...
export function SelectCardModel ({ label = __('Card model', 'teste-base'), value, onChange }) {
    const { data: settings } = useRestApi('teste-base/v2/block_settings');

    const options = useMemo(() => {
        const baseOptions = getOptions(settings?.cardModels);
        return [{ label: _x('Default model', 'card', 'teste-base'), value: '' }, ...baseOptions];
    }, [settings]);
