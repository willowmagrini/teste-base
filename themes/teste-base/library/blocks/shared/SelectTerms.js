...
export function SelectTerms ({ label = __('Terms', 'teste-base'), taxonomy, value, onChange }) {
    const [search, setSearch] = useState('');
    const setDebouncedSearch = useDebounce(setSearch, 500);

    const { data: terms } = useRestApi(`teste-base/v2/terms/${taxonomy}`);
    const { data: searchedTerms } = useRestApi(`teste-base/v2/terms/${taxonomy}`, { search });
